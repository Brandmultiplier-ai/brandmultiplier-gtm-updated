...
## BrandMultiplier GTM Overrides

This DESIGN.md is the base for BrandMultiplier GTM. Apply the full Claude (Anthropic) system above, with these project-specific overrides.

### Context
BrandMultiplier GTM is a private B2B outreach app (LinkedIn workflows via Unipile). Surfaces: Dashboard, Campaigns, Copilot, Unibox, Leads, Insights, Settings. It is **dashboard-dense** software, not a marketing site. Density and scan-speed matter as much as warmth.

### Density Adjustments
- App headings cap at 28–32px, never 52–64px (those are for marketing pages, not the product).
- Tighten section spacing: marketing-site `120px` becomes app `48–64px`.
- Tables and lists are first-class. Default row height 48px. Avatar 32px. Font: Sans 16px, not Serif.
- Reserve `Anthropic Serif` for: page titles, KPI numbers, card titles, empty states, modal headings. Everything else (table content, form labels, button text, metadata) is `Anthropic Sans`.

### Accent Override — Brand Orange
- Replace `Terracotta Brand (#c96442)` for **primary CTAs only** with a saturated, energetic orange:
  - **Brand Orange** `#FF5A1F` — primary buttons, "Send", "Launch campaign", "Approve", main CTA states, active sidebar accent
  - **Brand Orange Hover** `#E64A0F`
  - **Brand Orange Subtle** `#FFE8DD` — backgrounds for selected/active states, badges, signal pills
- Keep `Terracotta (#c96442)` and `Coral (#d97757)` for secondary accents, links, brand moments, illustrations, avatar gradients.
- Rationale: BrandMultiplier GTM lives in the same family as Gojiberry/HeyReach (warm orange CTAs). Pure Claude terracotta reads too editorial for action buttons in an outreach tool.

### Sidebar
- Background: `Deep Dark #141413`. Border-right: `#1f1e1c`.
- Inactive label: `Warm Silver #b0aea5`. Hover: `Parchment #f5f4ed` text on `#1f1e1c` background.
- Active item: `Parchment` text on `#1f1e1c` background **with 3px left border in `Brand Orange #FF5A1F`**.
- Section labels: 11px uppercase, `#5e5d59`, letter-spacing .08em.
- Width: 240px desktop, collapsible to 64px (icons only).

### Status & Semantic Colors (additions)
- **Success Green** `#3D8B5E` — accepted invitations, replied leads, active campaigns, positive deltas
- **Warning Amber** `#C58A2E` — paused campaigns, rate limit warnings
- **Error** uses Claude's `Error Crimson #b53333` unchanged
- **AI Score "Fire" gradient**: `#FF5A1F → #FFB347` (1–3 levels) for lead scoring icons

### BrandMultiplier GTM Components
- **Lead row**: avatar 32px + name (Sans 15px/500) + role · company (Sans 13px/400 Olive Gray) + signal tag (Sand background) + AI score icons + action menu. Hover: Parchment background.
- **KPI card**: Sans label (11px uppercase Stone Gray), Serif number (36px Near Black), Sans delta (13px Success/Error).
- **Campaign card**: Serif name (22px), Sans metadata, status pill, connect-rate + reply-rate inline as Serif 24px.
- **Unibox**: 3-col layout (320px / fluid / 320px). Inbound bubbles: Parchment. Outbound: Brand Orange Subtle.
- **Empty states**: Serif heading + organic illustration + Brand Orange CTA.

### Implementation Rule — Semantic Tokens Only

The token system is wired through `src/app/globals.css` `@theme inline` block. Tailwind v4 generates utility classes from these tokens. **Always use semantic utility classes, never hex literals or `rgba(...)` arbitrary values.**

Available semantic Tailwind utilities (use these instead of hex):

| Use case | Class | Maps to |
|---|---|---|
| Page background | `bg-background` | Parchment |
| Card / panel surface | `bg-card`, `bg-popover` | Ivory |
| Primary CTA | `bg-primary text-primary-foreground` | Brand Orange |
| Brand Orange direct | `bg-brand`, `text-brand`, `fill-brand` | `#FF5A1F` |
| Brand Orange hover | `bg-brand-hover` | `#E64A0F` |
| Brand Orange subtle bg | `bg-brand-subtle` | `#FFE8DD` |
| Brand accent (warm) | `text-terracotta`, `text-coral` | Claude warm |
| Body text | `text-foreground` | Near Black |
| Secondary text | `text-muted-foreground`, `text-olive` | Olive Gray |
| Tertiary / metadata | `text-stone` | Stone Gray |
| Light surface fill | `bg-muted`, `bg-sand` | Warm Sand |
| Borders | `border-border` | Border Cream |
| Success state | `text-success`, `bg-success` | Warm green |
| Warning state | `text-warning`, `bg-warning` | Warm amber |
| Error state | `text-destructive`, `bg-destructive` | Error Crimson |

For SVG `fill` / `stroke` attributes (where Tailwind utilities don't apply), use CSS vars directly: `fill="var(--brand-primary)"`, `stroke="var(--coral)"`.

**Anti-pattern checklist** — never write any of these in a component:
- `text-[#xxxxxx]` arbitrary hex
- `bg-[rgba(...)]` arbitrary rgba
- `text-white` / `text-black` (use `text-foreground` / `text-background`)
- `bg-zinc-*`, `bg-slate-*`, `text-emerald-400` (cool-tone Tailwind colors that bypass the warm system)
- Color literals in shadow utilities like `shadow-[0_0_8px_rgba(...)]`

If a color is needed that doesn't have a semantic token, **add the token first** in `globals.css` `@theme inline`, then use it. Never inline.

### Don'ts (BrandMultiplier GTM specific)
- Don't apply marketing-site spacing inside the app — it kills scan speed.
- Don't use Serif for table content, form labels, or button text.
- Don't put Brand Orange on more than one element per visible viewport area — it must remain the unique primary action signal.
- Don't introduce cool grays anywhere, even for "neutral" UI states. Stay warm.
- Don't use shadows for depth; use ring-shadows (`0 0 0 1px var(--border-cream)`) like Claude.
